...
app.config['MYSQL_DATABASE_PASSWORD'] = 'R0nanZ0sia01*' # add your root
...
def allTags():
	uid = getUserIdFromEmail(flask_login.current_user.id)
	cursor = conn.cursor()
	cursor.execute("SELECT DISTINCT T.word FROM Tags AS T, Pictures AS P WHERE P.user_id = '{0}' AND T.picture_id = P.picture_id".format(uid))
	all_tags = cursor.fetchall()
	return all_tags

@app.route("/viewTags", methods=['GET', 'POST'])
def viewTags():
	return render_template('viewTags.html', message='Browse all your tags', all_tags = allTags())
...
def hello():
	...
	all_comments = cursor.fetchall()
	cursor = conn.cursor()
	cursor.execute("SELECT T.word, P.picture_id FROM Pictures as P, Tags as T WHERE P.picture_id=T.picture_id")
	all_tags = cursor.fetchall()
	if (all_photos):
		return render_template('hello.html', message='Welcome to Photoshares home page', photos=all_photos, comments=all_comments, tags = all_tags, base64=base64, heading='The feed')
	return render_template('hello.html', message='Welcome to Photoshares home page')
...
def tag_page():
	return render_template('tagActivity.html', message='TOP used tags with the most pictures, (tag name, number of pictures using this tag)', TopTags = getAllTagAct())
...
def delete_picture():
	try:
		...
		pic_user_id = cursor.fetchall()[0]
		
		if user_id == pic_user_id:
			cursor = conn.cursor()
