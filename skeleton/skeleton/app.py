...
def hello():
	try: # this parts is for when a new comment is added
		render_template('hello.html', message='Welecome to Photoshares home page', photos=all_photos, base64=base64, heading='The feed')
		current_user = getUserIdFromEmail(flask_login.current_user.id)
		comment = request.form.get('new_comm')
		if comment != None:
			picture_id = 0 # passed picture_id from html code
			cursor = conn.cursor()
			cursor.execute("INSERT INTO Comments (comm_text, friend_email, picture_id) VALUES ('{0}', '{1}', '{2}')".format(comment, getUserEmailFromId(current_user), picture_id))
			conn.commit()
			cursor = conn.cursor()
			cursor.execute("SELECT P.imgdata, P.picture_id, P.caption, C.comm_text, P.user_id FROM Pictures as P, Comments as C WHERE P.picture_id = C.picture_id")
			all_photos = cursor.fetchall()
			return render_template('hello.html', message='Welecome to Photoshares home page', photos=all_photos, base64=base64, heading='The feed', currentUser = current_user)
		else: # the person is logged in but havent made a new comment
			cursor = conn.cursor()
			cursor.execute("SELECT P.imgdata, P.picture_id, P.caption, C.comm_text FROM Pictures as P, Comments as C ")
			all_photos = cursor.fetchall()
			if (all_photos):
